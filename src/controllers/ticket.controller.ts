...
export const myTicketQrCode = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (!ticket.qrToken)
      throw AppError.badRequest("This ticket has not been paid for yet");
...
export const verifyMyTicket = catchAsync(
  async (req: Request, res: Response) => {
    const ticket = await ticketService.verifyMyTicketPayment(
      req.user!.id,
      getParam(req, "id"),
    );
    res.status(200).json({
      ...
      data: { ticketId: ticket._id, paymentStatus: ticket.paymentStatus },
    });
  },
);

export const setMyTicketReminders = catchAsync(
  async (req: Request, res: Response) => {
    const ticket = await ticketService.setMyTicketReminders(
      req.user!.id,
      getParam(req, "id"),
      req.body.reminderOffsetsMinutes,
    );
    res.status(200).json({
      success: true,
      message: "Reminder preferences updated",
      data: { reminderOffsetsMinutes: ticket.reminderOffsetsMinutes },
    });
  },
);
